...
export const DeleteResourceModal: React.FC<DeleteResourceModalProps> = ({
  ...
  if (!isOpen || !resource || !resourceType) return null;

  const handleConfirm = () => {
    if (confirmName === resource.metadata.name) {
      onConfirm(resource, resourceType);
    }
  };
        ...
        <div className="mb-6">
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-4">
            <div className="mb-3">
              ...
              <div className="space-y-2 text-sm text-red-700">
                ...
                <div>
                  <strong>Version:</strong> {resource.metadata.version}
                </div>
                {resource.createdAt && (
                  <div>
                    <strong>Created:</strong> {formatDate(resource.createdAt)}
                  </div>
                )}
              </div>
            </div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              To confirm deletion, type the resource name:{" "}
